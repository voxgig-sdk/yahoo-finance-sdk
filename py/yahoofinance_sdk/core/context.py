...
from yahoofinance_sdk.core.control import YahooFinanceControl
...
from yahoofinance_sdk.core.helpers import get_ctx_prop, to_map
...
class YahooFinanceContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, YahooFinanceSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, YahooFinanceResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, YahooFinanceResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return YahooFinanceError(code, msg, self)
