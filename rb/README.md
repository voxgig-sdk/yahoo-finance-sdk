# YahooFinance Ruby SDK

The Ruby SDK for the YahooFinance API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Download(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.YahooFinance(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── YahooFinance_sdk.rb       -- Main SDK module
