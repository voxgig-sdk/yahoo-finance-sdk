...
class ScreenerEntity
  ...
  def make
    opts = @_entopts.dup
    ScreenerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YahooFinanceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Screener, Hash] the current Screener data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = YahooFinanceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Screener fields)
  def match_get
  ...
  #
  # @param reqdata [ScreenerCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Screener, Hash] the created Screener; raises YahooFinanceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = YahooFinanceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
