...
class TickerEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YAHOO_FINANCE_TEST_TICKER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function ticker_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["ticker01", "ticker02", "ticker03", "quote_summary01", "quote_summary02", "quote_summary03", "option01", "option02", "option03", "chart01", "chart02", "chart03"] as $k) {
    ...
    $entid_env_raw = getenv("YAHOO_FINANCE_TEST_TICKER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["YAHOO_FINANCE_TEST_TICKER_ENTID"]);
