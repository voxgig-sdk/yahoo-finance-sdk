...
module YahooFinanceUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = YahooFinanceContext.new({}, nil)
    end
    op = ctx.op || YahooFinanceOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || YahooFinanceResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(YahooFinanceError) ? err.msg : err.to_s
    ...
    sdk_err = YahooFinanceError.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(YahooFinanceError)
