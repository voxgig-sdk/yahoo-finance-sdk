...
class FtCtrl extends YahooFinanceControl
{
...
class FtClient
{
    ...
    private YahooFinanceUtility $utility;

    public function __construct(YahooFinanceUtility $utility, array $options)
    {
    ...
    public function get_utility(): YahooFinanceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YahooFinanceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YahooFinanceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YahooFinanceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YahooFinanceConfig::make_feature($name);
    ...
    private static function buildUrl(YahooFinanceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YahooFinanceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YahooFinanceResponse($fetched);
        }
    ...
    private function fail(YahooFinanceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YahooFinanceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
