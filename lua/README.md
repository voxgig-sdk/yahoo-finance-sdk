...
The Lua SDK for the YahooFinance API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Download(nil):load({ id = "example_id" }, nil)
...
local result, err = client:YahooFinance(nil):load(
  { id = "test01" }, nil
)
