# YahooFinance Lua SDK



The Lua SDK for the YahooFinance API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Download()` — each with the same small set of operations (`list`, `load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local download, err = client:Download():load({ id = "download_id", period1 = 1, period2 = 1 })
```
...
```
lua/
├── yahoo-finance_sdk.lua    -- Main SDK module
...
The main module (`yahoo-finance_sdk`) exports the SDK constructor
