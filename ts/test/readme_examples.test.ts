...
//     (\`new YahooFinanceSDK(...)\` / \`YahooFinanceSDK.test(...)\` become
//     \`YahooFinanceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'YahooFinanceSDK'
...
const TEST_SEED = {"entity":{"download":{"test01":{"id":"test01"}},"market":{"test01":{"id":"test01"}},"screener":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"ticker":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YahooFinanceSDK(...)\` and any
// \`YahooFinanceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YahooFinanceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YahooFinanceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
