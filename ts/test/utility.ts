...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.YAHOOFINANCE_TEST_LIVE ||
    'TRUE' === process.env.YAHOOFINANCE_TEST_OVERRIDE
  ) {
  ...
  m.YAHOOFINANCE_TEST_EXPLAIN = process.env.YAHOOFINANCE_TEST_EXPLAIN || m.YAHOOFINANCE_TEST_EXPLAIN
