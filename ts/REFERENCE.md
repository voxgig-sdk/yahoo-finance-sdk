# YahooFinance TypeScript SDK Reference

Complete API reference for the YahooFinance TypeScript SDK.
...
```ts
new YahooFinanceSDK(options?: object)
```
...
#### `YahooFinanceSDK.test(testopts?, sdkopts?)`
...
**Returns:** `YahooFinanceSDK` instance in test mode.
...
#### `Screener(data?: object)`

Create a new `Screener` entity instance.
...
**Returns:** `ScreenerEntity` instance.
...
Alias for `YahooFinanceSDK.test()`.
...
```ts
const result = await client.Download().load({ id: 'download_id', period1: 1, period2: 1 })
```
...
Return the parent `YahooFinanceSDK` instance.
