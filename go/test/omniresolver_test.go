...
import (
	...
	omni "github.com/voxgig-sdk/yahoo-finance-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YahooFinanceError); is {
				out := map[string]any{
					"name":    "YahooFinanceError",
...
func omniCtx(arg any, client *sdk.YahooFinanceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YahooFinanceSDK); is {
			client = live
