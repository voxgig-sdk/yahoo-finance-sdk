...
// YahooFinance SDK retry feature
...
class YahooFinanceRetryFeature extends YahooFinanceBaseFeature
{
    ...
    public function init(YahooFinanceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YahooFinanceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YahooFinanceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YahooFinanceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
