...
(0, node_test_1.describe)('DownloadDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.YahooFinanceSDK({
    ...
    (0, node_test_1.test)('direct-load-download', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-download', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.YAHOO_FINANCE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.YahooFinanceSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.YAHOO_FINANCE_APIKEY,
        }));
        let idmap = env['YAHOO_FINANCE_TEST_DOWNLOAD_ENTID'];
    ...
    const client = new __1.YahooFinanceSDK({
        base: 'http://localhost:8080',
