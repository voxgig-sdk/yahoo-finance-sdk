...
The Python SDK for the YahooFinance API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Download(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.YahooFinance(None).load(
    {"id": "test01"}, None
)
