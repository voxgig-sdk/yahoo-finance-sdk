...
local ScreenerEntity = {}
ScreenerEntity.__index = ScreenerEntity


function ScreenerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ScreenerEntity)
  self._name = "screener"
  self._client = client
...
function ScreenerEntity:get_name()
  return self._name
end


function ScreenerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ScreenerEntity.new(self._client, opts)
end
...
function ScreenerEntity:mark_deleted()
  self._deleted = true
end


function ScreenerEntity:deleted()
  return true == self._deleted
end


function ScreenerEntity:data_set(args)
  if args ~= nil then
...
function ScreenerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ScreenerEntity:match_set(args)
  if args ~= nil then
...
function ScreenerEntity:match_get()
...
function ScreenerEntity:stream(action, args, callopts)
...
function ScreenerEntity:_run_op(ctx, post_done)
  local utility = self._utility
