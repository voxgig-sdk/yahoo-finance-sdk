...
module YahooFinanceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YahooFinanceResponse.new({})
    ...
    if fetch_err
      ...
      response = YahooFinanceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YahooFinanceResponse.new(fetched)
    else
