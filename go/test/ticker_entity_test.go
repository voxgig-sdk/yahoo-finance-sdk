...
func TestTickerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set YAHOOFINANCE_TEST_TICKER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func tickerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YAHOOFINANCE_TEST_TICKER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YAHOOFINANCE_TEST_TICKER_ENTID"])
