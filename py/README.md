# YahooFinance Python SDK



The Python SDK for the YahooFinance API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Download()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/yahoo-finance-sdk/releases)) or
...
```python
download = client.Download().load({"id": "download_id", "period1": 1, "period2": 1})
```
...
```
py/
├── yahoofinance_sdk.py         -- Main SDK module
...
The main module (`yahoofinance_sdk`) exports the SDK class.
