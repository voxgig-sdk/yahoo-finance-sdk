...
boru-driven command-line client **and** interactive REPL for the YahooFinance
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yahoo-finance-cli)
make build

# 2. See usage (words, entities, env vars)
./yahoo-finance-cli --help

# 3. Provide credentials once, via the environment
export YAHOO_FINANCE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./yahoo-finance-cli load 1 download            # {id:1} shorthand
./yahoo-finance-cli load '{id:1}' download       # explicit match map

# 5. Override the API base URL for a single call
YAHOO_FINANCE_BASE=https://api.example.com ./yahoo-finance-cli load 1 download

# 6. No arguments -> interactive REPL
./yahoo-finance-cli
yahoo-finance> load 1 download
yahoo-finance> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yahoo-finance-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yahoo-finance-cli load 1 download
   ```
...
```sh
./yahoo-finance-cli load 1 download          # scalar shorthand for {id:1}
./yahoo-finance-cli load '{id:1}' download     # explicit match map
```
...
./yahoo-finance-cli load 1 download
```
...
Run with no arguments to open a REPL (prompt `yahoo-finance>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `download`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yahoo-finance-cli`. |
...
(`./yahoo-finance-cli <expr>`) and interactively (the REPL), and expressions compose the
