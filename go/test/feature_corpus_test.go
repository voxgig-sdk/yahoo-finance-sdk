...
//
// NewYahooFinanceSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.YahooFinanceSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewYahooFinanceSDK(opts)
}
...
func fcCandidates(client *sdk.YahooFinanceSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.YahooFinanceSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.YahooFinanceSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.YahooFinanceError
