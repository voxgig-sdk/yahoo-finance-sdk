// Command YahooFinance-cli is a CLI + REPL for the YahooFinance SDK,
...
const prompt = "yahoo-finance"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "download market screener search ticker"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      YAHOO_FINANCE_APIKEY (api key), YAHOO_FINANCE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: YAHOO_FINANCE_APIKEY carries the API key and
	// YAHOO_FINANCE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("YAHOO_FINANCE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("YAHOO_FINANCE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewYahooFinanceSDK(opts)
