...
[MCP](https://modelcontextprotocol.io) server exposing the YahooFinance SDK as
two agent tools — `yahoo-finance_list` and `yahoo-finance_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yahoo-finance-mcp)
make build

# 2. Provide credentials via the environment
export YAHOO_FINANCE_APIKEY=sk_live_xxx
...
```jsonc
// yahoo-finance_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// yahoo-finance_load: one record by id
{ "entity": "download", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yahoo-finance-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yahoo-finance \
     -- "$PWD"/dist/*/yahoo-finance-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yahoo-finance_list` and `yahoo-finance_load` tools now appear
   in new sessions. Ask the agent to *"list search using yahoo-finance"*
   and it calls `yahoo-finance_list` with `{"entity":"search"}`.
...
```sh
export YAHOO_FINANCE_APIKEY=sk_live_xxx            # API key
export YAHOO_FINANCE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `yahoo-finance_list` tool
...
### Call the `yahoo-finance_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `yahoo-finance_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yahoo-finance_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `YAHOO_FINANCE_APIKEY` | API key sent with every request. |
| `YAHOO_FINANCE_BASE` | Optional override of the API base URL. |
