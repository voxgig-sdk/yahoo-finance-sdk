...
class MarketEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YAHOOFINANCE_TEST_MARKET_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    market_ref01_data_dt0_loaded, err = market_ref01_ent.load(market_ref01_match_dt0, nil)
    assert_nil err
...
def market_basic_setup(extra)
  ...
  entid_env_raw = ENV["YAHOOFINANCE_TEST_MARKET_ENTID"]
