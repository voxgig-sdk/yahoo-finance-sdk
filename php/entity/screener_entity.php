...
// YahooFinance SDK Screener entity
...
class ScreenerEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ScreenerEntity($this->_client, $opts);
    }

    /**
     * @param Screener|array $args Screener data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = YahooFinanceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Screener|array The current Screener data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Screener fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = YahooFinanceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Screener fields).
     */
     ...
     *
     * @param ScreenerCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ScreenerCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Screener|array The created Screener as an assoc-array at the
     *   SDK boundary; throws YahooFinanceError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = YahooFinanceHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
