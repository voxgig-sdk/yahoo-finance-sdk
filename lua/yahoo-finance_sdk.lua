...
local YahooFinanceSDK = {}
YahooFinanceSDK.__index = YahooFinanceSDK
...
YahooFinanceSDK._make_feature = _make_feature


function YahooFinanceSDK.new(options)
  local self = setmetatable({}, YahooFinanceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function YahooFinanceSDK:options_map()
...
function YahooFinanceSDK:get_utility()
  return Utility.copy(self._utility)
end


function YahooFinanceSDK:get_root_ctx()
  return self._rootctx
end


function YahooFinanceSDK:prepare(fetchargs)
...
function YahooFinanceSDK:direct(fetchargs)
...
-- by passing it.
function YahooFinanceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function YahooFinanceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "YahooFinanceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function YahooFinanceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = YahooFinanceSDK.new(sdkopts)
  sdk.mode = "test"
