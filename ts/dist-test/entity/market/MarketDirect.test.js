...
(0, node_test_1.describe)('MarketDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-market', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-market', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['YAHOO_FINANCE_TEST_MARKET_ENTID'];
