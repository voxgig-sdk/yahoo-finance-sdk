...
// YahooFinance SDK pipeline test
//
...
{
    private static function utility(): YahooFinanceUtility
    {
        return new YahooFinanceUtility();
    }
    ...
    private static function ctx(array $over = []): YahooFinanceContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new YahooFinanceContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new YahooFinanceOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof YahooFinanceError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new YahooFinanceOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = YahooFinanceMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new YahooFinanceOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = YahooFinanceMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new YahooFinanceOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = YahooFinanceMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new YahooFinanceSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = YahooFinanceMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new YahooFinanceResponse([]),
            'result' => new YahooFinanceResult([]),
        ]);
        [, $err] = YahooFinanceMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new YahooFinanceSpec([]),
            'response' => null,
            'result' => new YahooFinanceResult([]),
        ]);
        [, $err] = YahooFinanceMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new YahooFinanceSpec(['step' => 's']),
            'response' => new YahooFinanceResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new YahooFinanceResult([]),
        ]);
        [, $err] = YahooFinanceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new YahooFinanceSpec(['step' => 's']),
            'response' => new YahooFinanceResponse(self::resp(200, ['v' => 1])),
            'result' => new YahooFinanceResult([]),
        ]);
        [, $err] = YahooFinanceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new YahooFinanceSpec(['step' => 's']),
            'response' => new YahooFinanceResponse(self::resp(200, ['v' => 2])),
            'result' => new YahooFinanceResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        YahooFinanceMakeResponse::call($ctx);
    ...
    {
        $preset = new YahooFinanceResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new YahooFinanceSpec([]),
            'response' => new YahooFinanceResponse([]),
            'result' => new YahooFinanceResult([]),
        ...
        [$got, $err] = YahooFinanceMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new YahooFinanceResult([])]);
        [, $err] = YahooFinanceMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new YahooFinanceSpec([]), 'result' => null]);
        [, $err] = YahooFinanceMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new YahooFinanceSpec(['step' => 's']),
            'result' => new YahooFinanceResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new YahooFinanceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = YahooFinanceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new YahooFinanceSpec(['step' => 's']),
            'result' => new YahooFinanceResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new YahooFinanceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = YahooFinanceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new YahooFinanceResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new YahooFinanceSpec([]),
            'result' => new YahooFinanceResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = YahooFinanceMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = YahooFinanceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new YahooFinanceError('boom', 'boom');
        $utility->fetcher = function (YahooFinanceContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new YahooFinanceSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = YahooFinanceMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (YahooFinanceContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = YahooFinanceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(YahooFinanceResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        YahooFinanceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (YahooFinanceContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new YahooFinanceSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = YahooFinanceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new YahooFinanceResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new YahooFinanceSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = YahooFinanceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = YahooFinanceMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = YahooFinanceMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new YahooFinanceResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, YahooFinanceDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new YahooFinanceResult(['ok' => false])]);
        $this->expectException(YahooFinanceError::class);
        YahooFinanceDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new YahooFinanceResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', YahooFinanceMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        YahooFinanceMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new YahooFinanceBaseFeature();
        $b = new YahooFinanceBaseFeature();
        YahooFinanceFeatureAdd::call($ctx, $a);
        YahooFinanceFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): YahooFinanceBaseFeature
    {
        $f = new YahooFinanceBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        YahooFinanceFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): YahooFinanceContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new YahooFinanceSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = YahooFinancePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        YahooFinancePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new YahooFinanceResponse(['headers' => null]),
            'result' => new YahooFinanceResult([]),
        ]);
        YahooFinanceResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        YahooFinanceResultBody::call($ctx);
